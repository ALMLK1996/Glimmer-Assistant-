...
[![Platform](https://img.shields.io/badge/Platform-Android%209%2B%20%7C%20iOS%2016%2B-69F0AE?style=for-the-badge&logo=android&logoColor=white)](https://developer.android.com)
[![Privacy](https://img.shields.io/badge/Privacy-Zero%20Data%20Leaves%20Device-brightgreen?style=for-the-badge)](docs/PRIVACY.md)
[![iOS](https://img.shields.io/badge/iOS-16%2B-000000?style=for-the-badge&logo=apple&logoColor=white)](https://developer.apple.com)
...
[![Author](https://img.shields.io/badge/Author-Abdulhannan%20Abdulkerim-FFD700?style=for-the-badge&logo=github&logoColor=black)](https://github.com/ALMLK1996)
...
- ✅ **100+ languages** — fully offline, including Arabic, English, French, Spanish, Chinese, Hindi, and more
- ✅ Language auto-detection — Glimmer responds in the language you speak
...
| Soul Type               | Example                                      |
|-------------------------|----------------------------------------------|
| 🌐 Multilingual Soul    | Understands and adapts to all languages and cultures naturally |
...
│       │   ├── MemoryStore.kt
│       │   └── HabitModel.kt
│       ├── shizuku/           # Elevated system access via Shizuku (no root)
│       │   ├── ShizukuBridge.kt
...
├── ios/
│   └── Glimmer/
│       ├── Core/              # Shared utilities, DI, theme base
│       ├── Overlay/           # Ghost overlay via UIWindow hierarchy
│       │   ├── GlimmerView.swift
│       │   ├── EmotionState.swift
│       │   └── FloatingController.swift
│       ├── AI/                # Local LLM core (llama.cpp / Core ML)
│       │   ├── LocalAI.swift
│       │   ├── NLUEngine.swift
│       │   └── ModelManager.swift
│       ├── Soul/              # Digital Soul system
...
│       └── Memory/            # Long-term memory (SwiftData + AES-256)
│           ├── MemoryStore.swift
│           └── HabitModel.swift
├── shared/                    # Cross-platform shared logic (C++ / AI core)
│   ├── ai_core/               # llama.cpp, Whisper.cpp, ONNX bindings
│   └── soul_format/           # GlimmerSoul JSON parser (shared)
├── docs/
...
│   │   └── feature_request.md
│   └── workflows/
│       ├── build-android.yml  # CI — Android automated build checks
│       ├── build-ios.yml      # CI — iOS automated build checks
│       └── release.yml        # CD — reproducible APK/IPA signing
├── CONTRIBUTING.md
├── SECURITY.md
├── CODE_OF_CONDUCT.md
├── CHANGELOG.md
...
| Layer                 | Technology                                                        |
|-----------------------|-------------------------------------------------------------------|
| **Languages**         | Kotlin · C++ (JNI/NDK for AI core) · Python (tooling & scripts)  |
...
| Layer                 | Technology                                                        |
|-----------------------|-------------------------------------------------------------------|
| **Languages**         | Swift · C++ (via Swift/C++ interop for AI core)                   |
| **UI Framework**      | SwiftUI + Metal (Custom Canvas rendering)                         |
| **Window System**     | UIKit overlay (persistent floating layer via `UIWindow` hierarchy)|
...
| **Soul Format**       | GlimmerSoul JSON (CC0 open standard — shared with Android)        |

> **Note:** iOS build is in active development. Core features (Ghost overlay, Local AI, Soul system) are available. Some system-level integrations (malware scanning, deep notification access) are limited by iOS sandboxing by design.
...
- Optional: [Shizuku](https://shizuku.rikka.app/) for elevated system features (no root required)
...
# 2. Open in Android Studio → File → Open → select glimmer/android/
...
# or: swift package resolve   # if using SPM

# 3. Open workspace in Xcode
open Glimmer.xcworkspace

# 4. Select target device and run (⌘+R)
```
...
```json
{
  ...
  "author": "optional",
  "language": "auto",
  "supported_languages": ["en", "ar", "fr", "es", "de", "zh", "hi", "tr", "ja", "ko"],
  "avatar": {
    "shape": "ghost",
    "colorPrimary": "#4FC3F7",
    "colorScheme": "rainbow",
  ...
  "responses": {
    "greeting": {
      "en": ["Hey!", "Hello there!", "What's up?"],
      "ar": ["أهلاً!", "مرحباً!", "كيف حالك؟"],
      "fr": ["Salut!", "Bonjour!", "Ça va?"],
      "es": ["¡Hola!", "¡Hey!", "¿Qué tal?"],
      "fallback": ["👋", "Hi!", "Hello!"]
    },
    "taskDone": {
      "en": ["Done!", "Got it!", "All set!"],
      "ar": ["تم!", "أنجزت!", "جاهز!"],
      "fallback": ["✅", "Done!"]
    },
    "error": {
      "en": ["Hmm, let me try again.", "Something went wrong."],
      "ar": ["حسناً، دعني أحاول مجدداً.", "حدث خطأ ما."],
      "fallback": ["⚠️", "Retry..."]
    }
  }
}
...
**Code standards:**
- Kotlin style via `ktlint` (Android)
- Swift style via `SwiftLint` (iOS)
- C++ follows LLVM coding standards (AI core)
...
<div align="center">
<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=700&size=14&duration=2200&pause=400&color=FF6D00&background=00000000&center=true&vCenter=true&multiline=false&repeat=true&width=560&height=36&lines=🔥+Phase+1+%E2%86%92+The+Soul;🧠+Phase+2+%E2%86%92+The+Mind;👁️+Phase+3+%E2%86%92+The+Senses;⚙️+Phase+4+%E2%86%92+The+Body;🌍+Phase+5+%E2%86%92+The+Community" alt="Roadmap animation"/>
</div>
...
- ✅ All distributions and forks **must** visibly credit the original author

**Original Author & Copyright Holder:**
> **Abdulhannan Abdulkerim**
> GitHub: [@ALMLK1996](https://github.com/ALMLK1996)
...
![Languages](https://img.shields.io/badge/Languages-Kotlin%20%7C%20Swift%20%7C%20C%2B%2B%20%7C%20Python-7C4DFF?style=flat-square&logo=kotlin&logoColor=white)
